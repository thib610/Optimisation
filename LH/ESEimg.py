...
def inneloop(X,nactp,nimp,J,M,th):
    ...
    for i in range(0,M):
        ...
        Xeval=maximin(X,DIM,NBpoint)
        if LHlisteval[indexXtry]-Xeval<=th*random.random():
            X=copy.deepcopy(Xtry)
            Xeval=LHlisteval[indexXtry]
            nactp+=1
        elif Xeval<maximin(Xbest,DIM,NBpoint):
            Xbest=X
            nimp+=1
        else:
            pass
